...
func TestInvocationValidate(t *testing.T) {
	...
	assert.NotNil((&Invocation{}).validate())
	assert.Nil((&Invocation{conn: &Connection{}}).validate())
}

func TestInvocationInvalidateCachedStatement(t *testing.T) {
	assert := assert.New(t)

	sc := NewStatementCache()
	sc.enabled = true
	sc.cache["foo"], _ = Default().PrepareContext(context.Background(), "select 'ok!'")
	sc.cache["bar"] = &sql.Stmt{}
	...
	assert.Nil(i.invalidateCachedStatement())
	assert.False(sc.HasStatement("foo"))
	assert.True(sc.HasStatement("bar"))
}

func TestInvocationInvalidateCachedStatementTx(t *testing.T) {
	assert := assert.New(t)

	sc := NewStatementCache()
	sc.enabled = true
	sc.cache["foo"] = &sql.Stmt{}
	sc.cache["bar"] = &sql.Stmt{}
...
func TestInvocationInvalidateCachedStatementEmptyLabel(t *testing.T) {
	assert := assert.New(t)
...
func TestInvocationInvalidateCachedStatementDisabled(t *testing.T) {
	assert := assert.New(t)
