...
func (kv1 KV1) Put(ctx context.Context, key string, data Values, options ...RequestOption) error {
	...
	req.Body = contents
	res, err := kv1.Client.send(req, TraceOptVaultOperation("kv1.put"), TraceOptKeyName(key))
	if err != nil {
		return err
	}
	defer res.Close()
...
func (kv1 KV1) Get(ctx context.Context, key string, options ...RequestOption) (Values, error) {
	...
	res, err := kv1.Client.send(req, TraceOptVaultOperation("kv1.get"), TraceOptKeyName(key))
	if err != nil {
		return nil, err
	}
	defer res.Close()
...
func (kv1 KV1) Delete(ctx context.Context, key string, options ...RequestOption) error {
	...
	res, err := kv1.Client.send(req, TraceOptVaultOperation("kv1.delete"), TraceOptKeyName(key))
	if err != nil {
		return err
	}
	defer res.Close()
...
func (kv1 KV1) List(ctx context.Context, path string, options ...RequestOption) ([]string, error) {
	...
	res, err := kv1.Client.send(req, TraceOptVaultOperation("kv1.list"))
	if err != nil {
		return nil, err
	}
	defer res.Close()
