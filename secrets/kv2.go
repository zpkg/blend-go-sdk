...
func (kv2 kv2) Put(key string, data Values, options ...Option) error {
	...
	req := kv2.client.createRequest(MethodPut, filepath.Join("/v1/secret/data", key), options...)
	req.Body = contents
...
func (kv2 kv2) Get(key string, options ...Option) (Values, error) {
	req := kv2.client.createRequest(MethodGet, filepath.Join("/v1/secret/data", key), options...)
	res, err := kv2.client.send(req)
	if err != nil {
		return nil, err
	}
...
func (kv2 kv2) Delete(key string, options ...Option) error {
	req := kv2.client.createRequest(MethodDelete, filepath.Join("/v1/secret/data", key), options...)
	res, err := kv2.client.send(req)
	if err != nil {
		return err
	}
