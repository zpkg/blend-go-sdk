...
func (kv2 KV2) Put(ctx context.Context, key string, data Values, options ...RequestOption) error {
	...
	req.Body = contents
	res, err := kv2.Client.send(req, TraceOptVaultOperation("kv2.put"), TraceOptKeyName(key))
	if err != nil {
		return err
	}
	defer res.Close()
...
func (kv2 KV2) Get(ctx context.Context, key string, options ...RequestOption) (Values, error) {
	...
	res, err := kv2.Client.send(req, TraceOptVaultOperation("kv2.get"), TraceOptKeyName(key))
	if err != nil {
		return nil, err
	}
	defer res.Close()
...
func (kv2 KV2) Delete(ctx context.Context, key string, options ...RequestOption) error {
	...
	res, err := kv2.Client.send(req, TraceOptVaultOperation("kv2.delete"), TraceOptKeyName(key))
	if err != nil {
		return err
	}
	defer res.Close()
...
func (kv2 KV2) List(ctx context.Context, path string, options ...RequestOption) ([]string, error) {
	...
	res, err := kv2.Client.send(req, TraceOptVaultOperation("kv2.list"))
	if err != nil {
		return nil, err
	}
	defer res.Close()
